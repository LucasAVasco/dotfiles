...
return {
	{
		...
		config = function(plugin, opts)
			...
			-- @param func Function that will be run after get focus on Trouble and before the jump
			...
			-- @param next If `true`, go to next position, otherwise go to previous
			...
			local modes = {'n', 'v'}
			vim.keymap.set(modes, '<leader>Tgg', decorator_focus_trouble_and_jump(trouble.first), key_options('Go to first trouble diagnostics'))
			vim.keymap.set(modes, '<leader>TG', decorator_focus_trouble_and_jump(trouble.last), key_options('Go to last trouble diagnostics'))
